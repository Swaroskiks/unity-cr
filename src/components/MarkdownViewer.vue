...
const LEGACY_BASES = [
  'http://51.77.147.126/api/images/images/',
  'http://51.77.147.126/api/audio/audio/',
  '/api/images/images/',
  '/api/audio/audio/'
];
...
const normalizeAssetUrl = (url: string) => {
  ...
  for (const base of LEGACY_BASES) {
    if (result.startsWith(base)) {
      return `${PROJECT_BASE}${result.slice(base.length)}`;
    }
  }

  if (/^(https?:|data:|blob:|#|\/)/i.test(result)) {
    return result;
  }

  result = result.replace(/^(\.\/)+/, '');
  result = result.replace(/^(\.\.\/)+/, '');
  return `${PROJECT_BASE}${result}`;
};

const rewriteHtmlAssetUrls = (html: string) => {
  return html.replace(/(src=)(['"])([^'"]+)\2/gi, (_match, prefix, quote, url) => {
    return `${prefix}${quote}${normalizeAssetUrl(url)}${quote}`;
  });
};
...
const processContent = async () => {
  ...
  const html = await marked(props.content, { renderer });
  renderedContent.value = rewriteHtmlAssetUrls(html);
  toc.value = newToc;
};
